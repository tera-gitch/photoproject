...
#デコレーターにより、CreatePhotoViewへのアクセスはログ井nユーザーに限定される
#ログイン状態でなければ、settings.pyのLOGIN_URLにリダイレクトされる
@method_decorator(login_required,name='dispatch')
class CreatePhotoView(CreateView):
    '''写真投稿ページのビュー
    PhotoPostFormで定義されているモデルとフィールドと連携して投稿データを
    データベースに登録する
...
class MypageView(ListView):
    ...
    def get_queryset(self):
        '''クエリを実行する
        self.kwargsの取得が必要なため、クラス変数querysetではなく、get_queryset()
        のオーバーライドによりクエリを実行する
...
class PhotoDeleteView(DeleteView):
    ...
    def delete(self,request,*args,**kwargs):
        ...
        Parameters:
            ...
            {'pk':21}の様にレコードのidが渡される
        
        Returns:
            HttpRespnseRedirect(success_url)を返して
            success_urlにリダイレクト
        '''
        #スーパークラスのdelete()を実行
